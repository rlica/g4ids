...
#ifndef FastPlasticHit_h
#define FastPlasticHit_h 1
...
class FastPlasticHit : public G4VHit
{
  public:

      FastPlasticHit();
     ~FastPlasticHit();
      FastPlasticHit(const FastPlasticHit&);
      const FastPlasticHit& operator=(const FastPlasticHit&);
      G4int operator==(const FastPlasticHit&) const;
...
typedef G4THitsCollection<FastPlasticHit> FastPlasticHitsCollection;

extern G4Allocator<FastPlasticHit> FastPlasticHitAllocator;

inline void* FastPlasticHit::operator new(size_t)
{
  void *aHit;
  aHit = (void *) FastPlasticHitAllocator.MallocSingle();
  return aHit;
}

inline void FastPlasticHit::operator delete(void *aHit)
{
  FastPlasticHitAllocator.FreeSingle((FastPlasticHit*) aHit);
}
