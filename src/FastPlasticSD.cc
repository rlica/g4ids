...
FastPlasticSD::FastPlasticSD(G4String name)
  :G4VSensitiveDetector(name), detectorID(-1), HCID(-1)
{
  HC_name = "FastPlastic";
  collectionName.insert(HC_name);
//  detectorID = AddFastPlasticDetector(name);
}
                                                                                                                                                                                                                         
FastPlasticSD::~FastPlasticSD()
{;}

void FastPlasticSD::Initialize(G4HCofThisEvent* evnt)
{
  hitsCollection = new FastPlasticHitsCollection(SensitiveDetectorName, HC_name);
...
G4bool FastPlasticSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
...
*/
  ...
  FastPlasticHit* newHit = new FastPlasticHit();
...
void FastPlasticSD::EndOfEvent(G4HCofThisEvent*)
{;}
               
/*
G4int FastPlasticSD::AddFastPlasticDetector(G4String name){
...
G4int FastPlasticSD::AddFastPlasticDetector(){
  G4int det_id = detectors.size();
  ostringstream id_strm;
  id_strm.clear();
  id_strm << det_id;
  return( AddFastPlasticDetector("detFastPlastic" + id_strm.str()) );
}
*/
