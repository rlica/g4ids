...
LaBr3SensitiveDetector::LaBr3SensitiveDetector(G4String name)
  :G4VSensitiveDetector(name), detectorID(-1), HCID(-1)
{
  HC_name = "LaBr3";
  collectionName.insert(HC_name);
//  detectorID = AddLaBr3Detector(name);
}
                                                                                                                                                                                                                         
LaBr3SensitiveDetector::~LaBr3SensitiveDetector()
{;}

void LaBr3SensitiveDetector::Initialize(G4HCofThisEvent* evnt)
{
  hitsCollection = new LaBr3HitsCollection(SensitiveDetectorName, HC_name);
...
G4bool LaBr3SensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
...
void LaBr3SensitiveDetector::EndOfEvent(G4HCofThisEvent*)
{;}
               
/*
G4int LaBr3SensitiveDetector::AddLaBr3Detector(G4String name){
...
G4int LaBr3SensitiveDetector::AddLaBr3Detector(){
  G4int det_id = detectors.size();
  ostringstream id_strm;
  id_strm.clear();
  id_strm << det_id;
  return( AddLaBr3Detector("detLaBr3" + id_strm.str()) );
}
*/
