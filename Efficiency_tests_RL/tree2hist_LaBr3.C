...
void tree2hist_LaBr3(char *FileName){
  ...
  TChain* tree = new TChain("idstree");
  tree->Add(FileName);
...
//      histoKU[cl*4+cr] = new TH1F("","",40000,1,10000);
  ...
  TH1F *hLaBr3Sum    = new TH1F("hLaBr3Sum" ,"LaBr3 Sum noAB",40000,1,10000);
  ...
  hClKUSum = new TH1F*[2];
  TH1F **hClBuchSum;
  hClBuchSum = new TH1F*[2];
  TH1F **hClKUSumAB;
  hClKUSumAB = new TH1F*[2];
  TH1F **hClBuchSumAB;
  hClBuchSumAB = new TH1F*[2];
  ...
  for (int i=0; i<5; i++) {
  	branch = Form("LaBr3_En%d",i);
  	cout << "Reading LaBr3 " << branch << endl;
 	tree->Draw(Form("%s>>h%s(40000,1,10000)",branch,branch),"","");
  	gROOT->ProcessLine(Form("h%s->Write();",branch));
  	tree->Draw(Form("%s>>hTemp",branch),"","");
  	hLaBr3Sum->Add(hTemp);
  }
